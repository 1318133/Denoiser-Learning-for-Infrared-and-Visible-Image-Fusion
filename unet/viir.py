...
class wtNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True, pthfile = 'F:/3line/checkpoints/CP_epoch13.pth'):
        ...
        self.e_conv1 = nn.Conv2d(1,int(number_f*0.5),3,1,1,bias=True,padding_mode='reflect') 

        self.ee_conv1 = nn.Conv2d(1,int(number_f*0.5),3,1,1,bias=True,padding_mode='reflect') 
    ...
    def forward(self, x, y):#(self, x, y):# x = RGB; y = NIR
        ...
        lossx,xx,nx = self.diffusion(out,x)
        lossy,yy,ny = self.diffusion1(out,y)


        return out,lossx,lossy,xx,yy,nx,ny # enhance_image_1(x0),enhance_image(y0),r(vis),out(img),img(inf)
